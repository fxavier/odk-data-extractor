    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.xml'):
            ...
            if existing_inquerito:
                print(
                    f"Skipping file {filename} as similar Inquerito already exists.")
                continue
            ...
            if agg_familiar_data:
                ...
                nome_proprietario = get_text(
                    'nome_proprietario', agg_familiar_data)
                codigo_familia = get_text('codigo_familia', agg_familiar_data)
                data_nascimento = get_text(
                    'data_nascimento', agg_familiar_data)
                celular = get_text('celular', agg_familiar_data)
                foto_proprietario = upload_image_to_s3(
                    get_text('foto_proprietario', agg_familiar_data), directory_path)
                casado = get_text('casado', agg_familiar_data)
                nome_conjugue = get_text('nome_conjugue', agg_familiar_data)
                data_nasciment_conjugue = get_text(
                    'data_nasciment_conjugue', agg_familiar_data)
                foto_conjugue = upload_image_to_s3(
                    get_text('foto_conjugue', agg_familiar_data), directory_path)
                nome_inquiridor = get_text(
                    'nome_inquiridor', agg_familiar_data)
                coordenadas_casa = get_text(
                    'coordenadas_casa', agg_familiar_data)
            ...
            if propriedade_data:
                proprietario_casa = get_text(
                    'proprietario_casa', propriedade_data)
                tipo_doc = get_text('tipo_doc', propriedade_data)
                outro_tipo_doc = get_text('outro_tipo_doc', propriedade_data)
                foto_documento = upload_image_to_s3(
                    get_text('foto_documento', propriedade_data), directory_path)
                bairro = get_text('bairro', propriedade_data)
                outro_bairro = get_text('outro_bairro', propriedade_data)
                quarteirao_localizada_familia = get_text(
                    'quarteirao_localizada_familia', propriedade_data)
                rua_localizada_familia = get_text(
                    'rua_localizada_familia', propriedade_data)
                numero_casa_familia = get_text(
                    'numero_casa_familia', propriedade_data)
            ...
            if caracteristicas_data:
                lingua_materna = get_text(
                    'lingua_materna', caracteristicas_data)
                outra_lingua = get_text('outra_lingua', caracteristicas_data)
                numero_de_pessoas_na_familia = get_text(
                    'numero_de_pessoas_na_familia', caracteristicas_data)
                if numero_de_pessoas_na_familia:
                    numero_de_pessoas_na_familia = int(
                        numero_de_pessoas_na_familia)
                membros_familia_count = get_text(
                    'membros_familia_count', caracteristicas_data)
                if membros_familia_count:
                    membros_familia_count = int(membros_familia_count)
                tempo_a_familia_vive_no_talhao = get_text(
                    'tempo_a_familia_vive_no_talhao', caracteristicas_data)
                numero_familias_talhao_terreno = get_text(
                    'numero_familias_talhao_terreno', caracteristicas_data)
                numero_mulheres_chefe_da_familia_tem = get_text(
                    'numero_mulheres_chefe_da_familia_tem', caracteristicas_data)
                ...
                for membro_data in caracteristicas_data.findall('membros_familia'):
                    ...
                    genero_membro_familia = get_text(
                        'genero_membro_familia', membro_data)
                    ...
                    grau_parentesco_com_chefe_familia = get_text(
                        'grau_parentesco_com_chefe_familia', membro_data)
                    nivel_educacao = get_text('nivel_educacao', membro_data)
                    doenca_cronica = get_text('doenca_cronica', membro_data)
                    tipo_doenca_cronica = get_text(
                        'tipo_doenca_cronica', membro_data)
                    outro_tipo_doenca_cronica = get_text(
                        'outro_tipo_doenca_cronica', membro_data)
                    deficiencia_fisica_mental = get_text(
                        'deficiencia_fisica_mental', membro_data)
            ...
            if propriedade_negocio_data:
                solucao_adoptar = get_text(
                    'solucao_adoptar', propriedade_negocio_data)
                data_nascimento_prop_negocio = get_text(
                    'data_nascimento_prop_negocio', propriedade_negocio_data)
                contacto_prop_negocio = get_text(
                    'contacto_prop_negocio', propriedade_negocio_data)
                doc_identificacao_prop_negocio = get_text(
                    'doc_identificacao_prop_negocio', propriedade_negocio_data)
                outro_doc_ident_prop_negocio = get_text(
                    'outro_doc_ident_prop_negocio', propriedade_negocio_data)
                photo_prop_negocio = upload_image_to_s3(
                    get_text('photo_prop_negocio', propriedade_negocio_data), directory_path)
                bairro_vive_prop_negocio = get_text(
                    'bairro_vive_prop_negocio', propriedade_negocio_data)
                bairro_localizacao_negocio = get_text(
                    'bairro_localizacao_negocio', propriedade_negocio_data)
                numero_senha_localizacao_negocio_gmaps = get_text(
                    'numero_senha_localizacao_negocio_gmaps', propriedade_negocio_data)
                coordenadas_estabelecimento_comercial = get_text(
                    'coordenadas_estabelecimento_comercial', propriedade_negocio_data)
                nome_comercial_negocio = get_text(
                    'nome_comercial_negocio', propriedade_negocio_data)
                tipo_negocio = get_text(
                    'tipo_negocio', propriedade_negocio_data)
                outro_tipo_negocio = get_text(
                    'outro_tipo_negocio', propriedade_negocio_data)
                proprietario_estrutura_comercial = get_text(
                    'proprietario_estrutura_comercial', propriedade_negocio_data)
                como_conseguiu_estrutura_comercial = get_text(
                    'como_conseguiu_estrutura_comercial', propriedade_negocio_data)
                outra_forma_conseguiu_est_comercial = get_text(
                    'outra_forma_conseguiu_est_comercial', propriedade_negocio_data)
                tempo_que_possui_estrutura_comercial = get_text(
                    'tempo_que_possui_estrutura_comercial', propriedade_negocio_data)
                tem_documento_que_confirma_propriedade_estabelecimento = get_text(
                    'tem_documento_que_confirma_propriedade_estabelecimento', propriedade_negocio_data)
                outro_doc_confirma_prop_estabelecimento = get_text(
                    'outro_doc_confirma_prop_estabelecimento', propriedade_negocio_data)
                foto_documento_estabelecimento = upload_image_to_s3(get_text(
                    'foto_documento_estabelecimento', propriedade_negocio_data), directory_path)
                tipo_material_construcao_estab_comercial_piso = get_text(
                    'tipo_material_construcao_estab_comercial_piso', propriedade_negocio_data)
                outro_tipo_material_est_come_piso = get_text(
                    'outro_tipo_material_est_come_piso', propriedade_negocio_data)
                tipo_material_construcao_estab_comercial_parede = get_text(
                    'tipo_material_construcao_estab_comercial_parede', propriedade_negocio_data)
                outro_tipo_material_est_come_parede = get_text(
                    'outro_tipo_material_est_come_parede', propriedade_negocio_data)
                tipo_material_construcao_estab_comercial_tecto = get_text(
                    'tipo_material_construcao_estab_comercial_tecto', propriedade_negocio_data)
                outro_tipo_material_construcao_tecto = get_text(
                    'outro_tipo_material_construcao_tecto', propriedade_negocio_data)
                tem_trabalhadores_envolvidos_negocio = get_text(
                    'tem_trabalhadores_envolvidos_negocio', propriedade_negocio_data)
                numero_trabalhadores = get_text(
                    'numero_trabalhadores', propriedade_negocio_data)
                valor_inicial_investido_no_seu_negocio = get_text(
                    'valor_inicial_investido_no_seu_negocio', propriedade_negocio_data)
                outro_valor_inicial_no_seu_negocio = get_text(
                    'outro_valor_inicial_no_seu_negocio', propriedade_negocio_data)
                rendimento_bruto_mensal_negocio = get_text(
                    'rendimento_bruto_mensal_negocio', propriedade_negocio_data)
                outro_rendimento_bruto_mensal_negocio = get_text(
                    'outro_rendimento_bruto_mensal_negocio', propriedade_negocio_data)
                lucro_medio_mensal_negocio = get_text(
                    'lucro_medio_mensal_negocio', propriedade_negocio_data)
                outro_lucro_medio_mensal_negocio = get_text(
                    'outro_lucro_medio_mensal_negocio', propriedade_negocio_data)
                outras_fontes_renda_negocio = get_text(
                    'outras_fontes_renda_negocio', propriedade_negocio_data)
            ...
            expectativa_compensacao_negocio_data = root.find(
                'expectativa_compensacao_prop_negocio')
            if expectativa_compensacao_negocio_data:
                compensacao_esperada_negocio = get_text(
                    'compensacao_esperada_negocio', expectativa_compensacao_negocio_data)
                localizacao_estrutura_receber = get_text(
                    'localizacao_estrutura_receber', expectativa_compensacao_negocio_data)
                outro_localizacao_estrutura_receber = get_text(
                    'outro_localizacao_estrutura_receber', expectativa_compensacao_negocio_data)
                outra_compensacao_esperada_negocio = get_text(
                    'outra_compensacao_esperada_negocio', expectativa_compensacao_negocio_data)
                foto_estrutura_receber = upload_image_to_s3(get_text(
                    'foto_estrutura_receber', expectativa_compensacao_negocio_data), directory_path)
                termo_consentimento_estrutura_negocio = upload_image_to_s3(get_text(
                    'termo_consentimento_estrutura_negocio', expectativa_compensacao_negocio_data), directory_path)
           ...
           # posse_terra_data = root.find
            ...
            if patrimonio_data:
                familia_possui_bens = get_text(
                    'familia_possui_bens', patrimonio_data)
                numero_de_fontes_de_renda = get_text(
                    'numero_de_fontes_de_renda', patrimonio_data)
                if numero_de_fontes_de_renda:
                    numero_de_fontes_de_renda = int(numero_de_fontes_de_renda)
                principais_fontes_de_rendas_count = get_text(
                    'principais_fontes_de_rendas_count', patrimonio_data)
                if principais_fontes_de_rendas_count:
                    principais_fontes_de_rendas_count = int(
                        principais_fontes_de_rendas_count)
                principais_fontes_de_rendas = get_text(
                    'principais_fontes_de_rendas', patrimonio_data)
                renda_media_mensal = get_text(
                    'renda_media_mensal', patrimonio_data)
            ...
            if servicos_publicos_data:
                donde_busca_agua_para_uso_na_familia = get_text(
                    'donde_busca_agua_para_uso_na_familia', servicos_publicos_data)
                outro = get_text('outro', servicos_publicos_data)
                quantidade_bidoes = get_text(
                    'quantidade_bidoes', servicos_publicos_data)
                if quantidade_bidoes:
                    quantidade_bidoes = int(quantidade_bidoes)
                tipo_tratamento_agua = get_text(
                    'tipo_tratamento_agua', servicos_publicos_data)
                tempo_levado_para_chegar_local_agua = get_text(
                    'tempo_levado_para_chegar_local_agua', servicos_publicos_data)
                como_vai_buscar_agua = get_text(
                    'como_vai_buscar_agua', servicos_publicos_data)
                quem_vai_buscar_agua = get_text(
                    'quem_vai_buscar_agua', servicos_publicos_data)
                principal_fonte_energia_usada_para_cozinha = get_text(
                    'principal_fonte_energia_usada_para_cozinha', servicos_publicos_data)
                tempo_para_chegar_local_compra_fonte_energia = get_text(
                    'tempo_para_chegar_local_compra_fonte_energia', servicos_publicos_data)
                meio_transporte_para_local_recolha_energia = get_text(
                    'meio_transporte_para_local_recolha_energia', servicos_publicos_data)
                fonte_energia_para_iluminacao_casa = get_text(
                    'fonte_energia_para_iluminacao_casa', servicos_publicos_data)
                outra_fonte_para_iluminacao_casa = get_text(
                    'outra_fonte_para_iluminacao_casa', servicos_publicos_data)
            ...
            if educacao_saude_data:
                existe_escola_no_bairro = get_text(
                    'existe_escola_no_bairro', educacao_saude_data)
                tem_membro_estudante = get_text(
                    'tem_membro_estudante', educacao_saude_data)
                quantos_membros = get_text(
                    'quantos_membros', educacao_saude_data)
                if quantos_membros:
                    quantos_membros = int(quantos_membros)
                escola_que_frequentam_criancas_familia_count = get_text(
                    'escola_que_frequentam_criancas_familia_count', educacao_saude_data)
                if escola_que_frequentam_criancas_familia_count:
                    escola_que_frequentam_criancas_familia_count = int(
                        escola_que_frequentam_criancas_familia_count)
                existe_centro_saude = get_text(
                    'existe_centro_saude', educacao_saude_data)
                nome_centro_saude = get_text(
                    'nome_centro_saude', educacao_saude_data)
                onde_procura_tratamento = get_text(
                    'onde_procura_tratamento', educacao_saude_data)
                tempo_para_chegar_local_tratamento = get_text(
                    'tempo_para_chegar_local_tratamento', educacao_saude_data)
                doencas_familia_sofre_mais = get_text(
                    'doencas_familia_sofre_mais', educacao_saude_data)
            ...
            estruturas_habitacionais_data = root.find(
                'estruturas_habitacionais')
            if estruturas_habitacionais_data:
                a_casa_que_vives_e = get_text(
                    'a_casa_que_vives_e', estruturas_habitacionais_data)
                comprimento_talhao = get_text(
                    'comprimento_talhao', estruturas_habitacionais_data)
                if comprimento_talhao:
                    comprimento_talhao = float(comprimento_talhao)
                largura_talhao = get_text(
                    'largura_talhao', estruturas_habitacionais_data)
                if largura_talhao:
                    largura_talhao = float(largura_talhao)
                de_que_material_feita_vedacao_talhao = get_text(
                    'de_que_material_feita_vedacao_talhao', estruturas_habitacionais_data)
                outro_material_vedacao = get_text(
                    'outro_material_vedacao', estruturas_habitacionais_data)
                quantas_estruturas_tem_o_talhao = get_text(
                    'quantas_estruturas_tem_o_talhao', estruturas_habitacionais_data)
                if quantas_estruturas_tem_o_talhao:
                    quantas_estruturas_tem_o_talhao = int(
                        quantas_estruturas_tem_o_talhao)
                estruturas_talhaos_count = get_text(
                    'estruturas_talhaos_count', estruturas_habitacionais_data)
                if estruturas_talhaos_count:
                    estruturas_talhaos_count = int(estruturas_talhaos_count)
                quanto_tempo_tem_casa_principal = get_text(
                    'quanto_tempo_tem_casa_principal', estruturas_habitacionais_data)
                familia_tem_duat_talhao = get_text(
                    'familia_tem_duat_talhao', estruturas_habitacionais_data)
                familia_possui_outra_estrutura_fora_deste_talhao = get_text(
                    'familia_possui_outra_estrutura_fora_deste_talhao', estruturas_habitacionais_data)
            ...
            if estruturas_talhaos_data:
                estruturas_talhao = get_text(
                    'estruturas_talhao', estruturas_talhaos_data)
                estrutura_sera_afectada = get_text(
                    'estrutura_sera_afectada', estruturas_talhaos_data)
                codigo_estrutura_afectada = get_text(
                    'codigo_estrutura_afectada', estruturas_talhaos_data)
                numero_divisoes = get_text(
                    'numero_divisoes', estruturas_talhaos_data)
                if numero_divisoes:
                    numero_divisoes = int(numero_divisoes)
                nivel_afectacao_estrutura = get_text(
                    'nivel_afectacao_estrutura', estruturas_talhaos_data)
                ...
                material_cobertura_estruturas = get_text(
                    'material_cobertura_estruturas', estruturas_talhaos_data)
                outro_material_tecto = get_text(
                    'outro_material_tecto', estruturas_talhaos_data)
                material_das_paredes = get_text(
                    'material_das_paredes', estruturas_talhaos_data)
                material_de_soalho_das_estruturas = get_text(
                    'material_de_soalho_das_estruturas', estruturas_talhaos_data)
                outro_material_soalho = get_text(
                    'outro_material_soalho', estruturas_talhaos_data)
                foto_esboco = upload_image_to_s3(
                    get_text('foto_esboco', estruturas_talhaos_data), directory_path)
                estruturas_talhaos = EstruturasTalhaos(
                    # Assuming you've created an instance of EstruturasHabitacionais before this
                    estruturas_habitacionais=estruturas_habitacionais,
            ...
            religiao_e_locais_sagrados_data = root.find(
                'religiao_e_locais_sagrados')
            if religiao_e_locais_sagrados_data:
                religiao = get_text(
                    'religiao', religiao_e_locais_sagrados_data)
                tempo_chegar_igreja = get_text(
                    'tempo_chegar_igreja', religiao_e_locais_sagrados_data)
                onde_enterra_entequeridos = get_text(
                    'onde_enterra_entequeridos', religiao_e_locais_sagrados_data)
                familia_tem_campas = get_text(
                    'familia_tem_campas', religiao_e_locais_sagrados_data)
                frequencia_com_que_vai_cemiterio = get_text(
                    'frequencia_com_que_vai_cemiterio', religiao_e_locais_sagrados_data)
            ...
            if pecuaria_data:
                familia_cria_animais = get_text(
                    'familia_cria_animais', pecuaria_data)
                familia_possui_arvores = get_text(
                    'familia_possui_arvores', pecuaria_data)
            ...
            if arvore_fruta_data:
                arvore_de_frutas = get_text(
                    'arvore_de_frutas', arvore_fruta_data)
                outra_arvore_de_fruta = get_text(
                    'outra_arvore_de_fruta', arvore_fruta_data)
                quantidade_arvores = get_text(
                    'quantidade_arvores', arvore_fruta_data)
                idade_arvores = get_text('idade_arvores', arvore_fruta_data)
                tempo_producao_frutas = get_text(
                    'tempo_producao_frutas', arvore_fruta_data)
            ...
            if resolucao_conflitos_data:
                a_quem_recorre_em_caso_de_conflitos = get_text(
                    'a_quem_recorre_em_caso_de_conflitos', resolucao_conflitos_data)
                metodo_para_receber_informacao = get_text(
                    'metodo_para_receber_informacao', resolucao_conflitos_data)
                metodo_para_dar_informacao = get_text(
                    'metodo_para_dar_informacao', resolucao_conflitos_data)
            ...
            if expectativas_tratamento_data:
                ao_sair_do_lugar_o_que_espera_como_compensacao = get_text(
                    'ao_sair_do_lugar_o_que_espera_como_compensacao', expectativas_tratamento_data)
                outra_compensacao_esperada = get_text(
                    'outra_compensacao_esperada', expectativas_tratamento_data)
                onde_gostaria_de_ser_reassentado = get_text(
                    'onde_gostaria_de_ser_reassentado', expectativas_tratamento_data)

                # Processing images for foto_esboco_estrutura
                foto_esboco_estrutura_name = upload_image_to_s3(get_text(
                    'foto_esboco_estrutura', expectativas_tratamento_data), directory_path)

                # Processing images for termo_consentimento
                termo_consentimento_name = upload_image_to_s3(
                    get_text('termo_consentimento', expectativas_tratamento_data), directory_path)
